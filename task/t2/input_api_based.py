...
def retrieve_context(user_question: str) -> list[dict[str, Any]]:
    ...
    #TODO:
    pydantic_output_parser = PydanticOutputParser(pydantic_object=SearchRequests)

    messages = [
        SystemMessagePromptTemplate.from_template(QUERY_ANALYSIS_PROMPT).format_prompt(
    ...
    search_requests: SearchRequests = (prompt | llm_client | pydantic_output_parser).invoke({})
    ...
    if search_requests.search_request_parameters:
        ...
        print(f"Search parameters extracted: {requests_dict}")
