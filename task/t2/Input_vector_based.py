...
class UserRAG:
    ...
    async def _create_vectorstore_with_batching(self, documents: list[Document], batch_size: int = 100):
        ...
        for vectorstore in batch_result:
            if final_vectorstore is None:
                final_vectorstore = vectorstore
            else:
                final_vectorstore = FAISS.merge_from([final_vectorstore, vectorstore])
        
        if final_vectorstore is None:
            raise ValueError("No vectorstores were created from the documents.")    
